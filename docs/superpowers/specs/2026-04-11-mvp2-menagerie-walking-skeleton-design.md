...
2. **手写 `hardware/elrobot/simulation/elrobot_follower.xml`**：
   ...
   - 3 个 ElRobot 独有关节（预计是 shoulder roll / shoulder yaw / wrist yaw 里的某几个，取决于 Menagerie 实际 5 关节的 axis 分布）：nearest-neighbor 估值
...
**Menagerie SO-ARM100**（confirmed 2026-04-11）: **5 revolute + 1 gripper = 6 actuators**. Last tuning 2025-06-09, joint limits matched to real hardware. **No SO-101 variant** in Menagerie (upstream `TheRobotStudio/SO-ARM100/Simulation/SO101` has 6-DOF MJCF but auto-generated, not hand-tuned).
...
1. **3 个 ElRobot 独有 joint**（从 M2 / M3 / M5 / M7 里挑，取决于 Menagerie 的 5 关节 axis 实际是什么）的 `armature` / `damping`：nearest-neighbor 估值，spec comment 标注 "estimated, subject to tuning"
...
- **Menagerie trs_so_arm100** (fork base): https://github.com/google-deepmind/mujoco_menagerie/tree/main/trs_so_arm100
...
- **SO-ARM100 upstream**: https://github.com/TheRobotStudio/SO-ARM100
- **SO-101 upstream MJCF** (6-DOF, auto-generated, not hand-tuned): https://github.com/TheRobotStudio/SO-ARM100/tree/main/Simulation/SO101
- **Architecture reference**: https://github.com/lachlanhurst/so100-mujoco-sim — MIT-licensed MuJoCo + Menagerie SO-ARM100 + LeRobot control + Qt UI. Structurally equivalent to MVP-2 modulo the UI layer and bridge; Phase 0 Task 1.1 Step 7 reads this repo as implementation hint source.
- **LeRobot** (v0.5.1, 2026-04-07): https://github.com/huggingface/lerobot — MVP-3 env-wrapper target, not MVP-2 dependency
- **LeIsaac** (SO-101 in Isaac Lab + HDF5→LeRobotDataset pattern): https://github.com/LightwheelAI/leisaac — reference for eventual MVP-3+ env wrapper distribution via HF EnvHub
