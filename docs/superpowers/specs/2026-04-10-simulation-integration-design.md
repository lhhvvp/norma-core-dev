...
- **`st3215-compat-bridge`**（`software/sim-bridges/st3215-compat-bridge/` 新 crate）：双向 Bridge。上行订阅 `st3215/commands` 翻译为 `ActuationBatch`；下行订阅 `WorldSnapshot` 用 `st3215-wire` 打包为 71 字节 dump 写进 `st3215/inference`。下游 web UI 和 station_py 零修改。
...
```
 ...
 │       ├─ command_task (st3215/commands → ActuationBatch)          │
...
│   ├── child_process.rs    # ChildProcessBackend
│   ├── external_socket.rs  # ExternalSocketBackend
...
```
现有 st3215 driver          →  st3215-wire
─────────────────────────      ──────────────────
src/protocol/memory.rs         register.rs + layout.rs
src/protocol/units.rs          units.rs
src/port.rs::parse_*           unpack.rs（纯 parsing 部分）
inline 常量                    presets.rs::ST3215_STANDARD
```

留在 `st3215` 的是**串口 I/O + 主控制循环 + NormFS 注册 + auto-calibration**——这些是 real driver 的 tokio/io 密集部分，不是纯协议。
...
│   ├── command_task.rs                    # 任务 A: st3215/commands → ActuationBatch
...
1. **command_task**：
   - 订阅 `st3215/commands` queue（既有 NormFS queue，schema 不变）
   - 对每条 legacy ST3215 命令：查 `actuator_map` → 构造 `ActuationCommand{set_position / torque_enable / ...}`
   - 根据命令类型路由到 QoS lane：`SetPosition` → `LOSSY_SETPOINT`；`TorqueEnable/Reset` → `RELIABLE_CONTROL`
   - 调 `sim_runtime.send_actuation(batch).await`
   - 错误处理：unknown motor_id → log warn 单条失败继续；lane 溢出 → log error
...
3. `config.legacy_bus_serial` 不能和任何 real driver 已注册的 bus serial 冲突——检测方式：启动时扫 `st3215/meta` queue 看已发布的 bus serial。冲突 → `BridgeError::BusSerialCollision`
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    station.shutdown_drivers().await?;
...
pub struct Station {
    // ... 既有字段 ...
    pub config: Config,                             // 既有（v1 spec 已要求改 pub）
    pub normfs: Arc<NormFS>,                        // 既有
    pub engine: Arc<dyn StationEngine>,             // 既有
...
message Hello {
  ...
  string client_role = 2;               // "station-runtime" / ...
  string client_id = 3;
}
...
**原则**：idempotent + 带 self-check。反复跑结果一致；自检失败 → exit 1。

**Self-check**：
- 每个 `actuator_id` 在 MJCF 有对应 actuator 元素
...
**删除** v1 的 `Drivers::validate()` mutual exclusion 规则。
...
**删除**：v1 spec §6.2 的 `Drivers::validate()` + `MutuallyExclusive` error variant。
...
inference:
  ...
  - queue-id: inference/normvla-real
    format: normvla
    st3215-bus: "ST3215-BUS-A1B2C3"             # 真机 EEPROM 序列号
    update-interval: 100ms
  # sim 回路
...
**这在 v1 spec 下是被 `MutuallyExclusive` 硬拒绝的**。v2 是第一类用例。
...
**`st3215-compat-bridge` crate**（≥ 8 测试）：

- `test_preset_loader_happy`
- `test_preset_loader_invalid_yaml`
- `test_actuator_map_bidirectional`
- `test_command_task_setpos_translates`
- `test_command_task_torque_enable_routes_reliable`
- `test_state_task_packs_bytes_correctly` — mock SimulationRuntime 发 WorldSnapshot，验证 inference queue 写入内容
...
| Q2 | `cargo test -p st3215-compat-bridge` 全绿（≥ 8 测试）|
...
| **`St3215CompatProjection` 作为单向只读组件** | 改为 `St3215CompatBridge` 双向——命令流从 `st3215/commands` 进 bridge 也要翻译 |
